...
export async function handleGoogleSignIn() {
    signInWithPopup(auth, provider)
        .then(async (result) => {
            ...
            const idToken = await user.getIdToken(true); // Ensure you get a fresh ID token

            // Fetch user data from Firestore using the ID token
            const headers = {
                "Authorization": "Bearer " + idToken,
            };

            fetch(`https://firestore.googleapis.com/v1/projects/finnovators-bb552/databases/(default)/documents/userData/${user.uid}`, {
                method: 'GET',
                headers: headers,
            })
